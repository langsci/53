...
is conveyed by the phrase ``von ... aus'' (from .. your perspective).
...
to this question is that spatial language is a \emph{complex adaptive system}\index{complex adaptive system}
...
This book defends the \emph{selectionist theory of language evolution},
...
in communication (\emph{communicative success}), 
how complex they are in processing (\emph{cognitive effort}) and 
other factors. 
...
necessitate a \emph{whole systems approach}\index{whole systems approach}
...
a \emph{spatial relation}, which is combined with a \emph{landmark} and 
a \emph{perspective}.
...
``rechts'' (right), ``vorne'' (front), ``nah'' (near), 
``n\"ordlich'' (north on and so forth).
...
In the Example phrase, ``rechts'' (right) is the locative spatial relation.
In this book we study three classes of spatial relations. 
\emph{Proximal} relations are based on distance estimations. 
Examples of proximal relations in German are ``nah'' (near) and ``fern'' (far).
The second class is called \emph{projective}
relations and includes direction-based spatial relations such as ``links'' (left) and ``vor'' (front).
The last class considered are absolute relations such as ``n\"ordlich''
(north) and ``\"ostlich'' (east). These are also direction-based, but the direction
...
In the Example \ref{e:der-block-rechts-der-kiste-von-dir-aus}, the landmark 
is expressed in the determined noun phrase 
``der Kiste'' (the box) immediately following the spatial relation.
...
and auditory capacities. The second question is one about how much the biological basis 
...
Roughly 30 years ago vocabulary such as ``email'' or ``website'' did not even exist.
...
``n\"ordlich'' (north), ``s\"udlich'' (south). These spatial
...
German locative systems, effectively consists of different conceptualization
...
system the \emph{language strategy} \citep{steels2011self-organization}.
Language strategies have a \emph{linguistic} part and a \emph{conceptual} 
...
An example from biology for self-organization is swarm behavior in school of fish. 
...
private representations of the language that he speaks and he can 
adjust his own representations based on local interactions with peers. 
...
the behavior undergoes, but also what is the ontogenetic bases 
...
How did syntax and semantics the language system under consideration co-evolve over time?
\end{description}
...
\label{s:stages}
One way of understanding evolutionary processes is to try and identify
...
This book orients itself alongside \cite{steels2005emergence}\index{Steels, L.} which proposes
a number of stages of complexity which are relevant for this book:
\emph{single-word utterances}, \emph{multi-word utterances}, \emph{grammatical utterances}
(see also Figure \ref{f:co-evolution-complexity}). 
...
directly refer to spatial regions such as ``links'' (left) or ``n\"ordlich'' (north).
...
mark landmarks. An example utterance is ``links Kiste'' (left box) which is used 
...
Imagine an utterance like ``Kiste link'' (box left) without the grammatical information, in particular, 
without word order and lexical class information. In that case a hearer does not know whether 
``link'' is an adjective or an adverb. This syntactic underdetermination has consequences for 
the semantic interpretation. If the phrase is interpreted as an adjective noun phrase as in 
``linke Kiste'' (left box), the spatial category acts as a modifier on the set of boxes. If the 
spatial relation is interpreted as an adverb, then box might be a landmark and the whole phrase 
denotes a region next to the landmark as in ``links der Kiste'' (to the left of the box).
...
many different categorization systems: projective, e.g. ``links'' (left) or ``rechts'' (right), 
proximal, e.g. ``nah'' (near) and ``fern'' (far), and absolute, e.g. ``n\"ordlich (north) 
and ``s\"udlich'' (south). This is one aspect. The other aspect is that
...
This book is structured into three main big parts besides this introduction 
and the conclusion. The first main part explains the interaction
model and the technical systems needed for studying spatial language.
The second part deals with objective number one and details the reconstruction
efforts for the German locative system. In the third part I detail how spatial
...
\paragraph*{Spatial Language Games}
...
\paragraph*{Embodied cognitive semantics with IRL}
...
\paragraph*{Construction grammar with FCG}
...
To ground the modelling efforts in sufficient knowledge of a real spatial language system, 
...
\paragraph*{Syntactic processing}
...
\paragraph*{Conceptualization of spatial scenes}
...
\paragraph*{Integrating syntactic and semantic processing}
...
\paragraph*{Acquisition and formation of basic spatial category systems} 
...
When an agent has to invent a new category he uses the strategy that is most
...
\paragraph*{Origins and alignment of spatial conceptualization strategies}
...
\paragraph*{Multi-word lexical systems for expressing landmarks}
...
\paragraph*{Grammar as a tool for disambiguating spatial phrases}
The part on language evolution of this book is concluded by a chapter that examines
