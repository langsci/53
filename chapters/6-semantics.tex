...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Representing spatial relations}\index{spatial relation}
The semantics of spatial relations is the subject of ongoing debate. The 
key question is whether there is something like a 
\textsc{semantic core}\index{spatial relation!semantic core}  of 
...
(for a similar approach to color, see \citealp{bleys2010phd}\index{Bleys, J.}). 
...
which represent prototypical distances (see Figure \ref{f:category-hierarchy} for an overview).\index{spatial relation!proximal}
...
Projective and absolute relations are\index{spatial relation!projective}\index{spatial relation!absolute}\index{spatial relation!angular}
...
in similarity the bigger the angular distance\todo{I think ``the bigger the angular distance'' should be omitted}, whereas low values for $\sigma$ 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Applying spatial relations}\index{spatial relation!regions}
...
and {\footnotesize\tt object-class} described in \sectref{s:irl}. 
...
the object (see \sectref{s:irl} for description). 
...
\section{Frames of reference}\index{frames of reference}
...
\begin{description}
\item[Intrinsic frame of reference]\index{frames of reference!intrinsic}
...
induced on the scene by a marker.\index{frames of reference!relative}}
...
\index{frames of reference!absolute}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Internal and external regions}\index{spatial relation!internal}\index{spatial relation!external}
...
\section{Group-based reference}\index{spatial relation!group-based reference}
...
called \emph{k-means}\index{k-means} \citep{lloyd1982least}\index{Lloyd, S.P.} in order to find the group 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Perspective marking}\index{perspective marking}
...
\subsection{Functional constraints}\index{spatial relation!functional constraints}
...
\subsection{Contextual factors}\index{spatial relation!contextual factors}
...
\emph{prototypical size} \citep{gapp1994basic}\index{Gapp, K.}, but also 
\emph{object salience} \citep{regier2001grounding}\index{Regier, T.}\index{Carlson, L. A.}, and \emph{object interference} 
...
(example adapted from \citealp{gapp1994basic}\index{Gapp, K.}) by the difference in typical 
...
\subsection{Other modeling approaches}\index{spatial relation!spatial ontologies}\index{spatial relation!qualitative spatial reasoning}
