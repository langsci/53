...
(see \figref{f:discrimination-vs-description} for an example). 
...
(see \sectref{s:irl}) as a means to 
...
The \textsc{discrimination score} \index{discrimination score}
...
be called \textsc{strict category membership}\index{categorization!strict} (see \citealp{belpaeme2007language}\index{Bleys, J.}\index{Belpaeme, T.} 
...
(see \figref{f:voronoi}). Applying such an approach to discrimination, one needs to additionally 
...
(see the \textsc{shifting contrast principle} \index{shifting contrast principle|see{greatest distance principle}}introduced by 
...
values (see for example \citealp{barwise1981generalized}\index{Cooper, R.}\index{Barwise, J.}) -- in other words, where noun phrases denote sets of objects for which
...
facing the problem of \textsc{perceptual deviation}.\index{perceptual deviation}
...
Lenient versus strict categorization\index{categorization!strict}.
