...
% communicative goal
\paragraph*{Influence of type of communicative goal -- which vs where}
...
\citep{tenbrink2005identifying}\index{Tenbrink, T.} 
...
\begin{figure}
\begin{centering}
\includegraphics[width=0.3\columnwidth]{figs/conceptualization-in-front-of-the-box-from-my-perspective-region.png}
\hspace{1pt}
\includegraphics[width=0.3\columnwidth]{figs/conceptualization-right-of-me-from-your-perspective-region.png}
\hspace{1pt}
\includegraphics[width=0.3\columnwidth]{figs/conceptualization-left-of-me-region.png} \\
\vspace{3pt}
\includegraphics[width=0.3\columnwidth]{figs/conceptualization-right-of-the-box-region.png}
\hspace{1pt}
\includegraphics[width=0.3\columnwidth]{figs/conceptualization-near-the-box-region.png}
\hspace{1pt}
...
\paragraph*{Ranking of Semantic Structure.}
...
The \emph{discrimination score} \index{discrimination score}
...
seems at least contradictory (see \citealp{miller1976language}\index{Johnson-Laird, P. N.}\index{Miller, G. A.} for 
...
and their preference by scoring them accordingly \citep{gerasymova2010acquisition}\index{Gerasymova, K.}\index{Spranger, M.}.\index{measures!conceptualization strategy similarity}
...
%\paragraph*{Additional Influences on Conceptualization not incorporated}
...
be called \emph{strict category membership}\index{categorization!strict} (see \citealp{belpaeme2007language}\index{Bleys, J.}\index{Belpaeme, T.} 
...
a process known as Voronoi tesselation \citep{aurenhammer1991voronoi}\index{Aurenhammer, F.} 
...
contrast} \index{greatest distance principle}\index{greatest contrast|see{greatest distance principle}} 
...
(see the \emph{shifting contrast principle} \index{shifting contrast principle|see{greatest distance principle}}introduced by 
\citealp{herskovits1986language}\index{Herskovits, A.} and similar ideas in \citealp{freksa1999links}\index{Freksa, C.}).
...
The second line of arguments against the \emph{strict category membership} 
approach to discrimination comes from computational modeling. In particular, we
can compare the approach advocated in this book with the \emph{strict category membership} \index{strict category membership|see{categorization!strict}}
...
\citealp{aurenhammer1991voronoi}\index{Aurenhammer, F.}).}
\label{f:voronoi}
\end{figure}

\paragraph*{Strict Category Membership}
...
values (see for example \citealp{barwise1981generalized}\index{Cooper, R.}\index{Barwise, J.}). 
...
\citealp{eschenbach1997axiomatic}\index{Eschenbach, C.}\index{Kulik, L.} for an example). Consequently
...
facing the problem of \emph{perceptual deviation}.\index{perceptual deviation}
...
\end{center}
\caption[Lenient categorization semantics]{\index{categorization!lenient}
...
While the target entity of the semantic structure in Figure \ref{f:filtering-operations} 
...
\end{center}
\caption[Lenient vs strict categorization]{\index{categorization!strict}
Lenient versus strict categorization\index{categorization!strict}. The top figure shows the 
...
{\index{categorization!strict}\index{categorization!lenient}
...
spatial scenes, whereas the success of strict categorization\index{categorization!strict} drops to 22\%
...
{Results of comparing strict versus lenient categorization\index{categorization!lenient}
...
effect on the strict categorization\index{categorization!strict} approach.}
...
adding more categories to the inventory of strict categorization\index{categorization!strict} agents.
...
{Results of comparing different sets of spatial categories and their 
effect on the lenient categorization approach.\index{categorization!lenient}}
\label{f:apply-increased-categories}
\end{figure}

In some cases most notably for data set \emph{space-game-18} 
...
number of categories\index{measures!number of categories}. The most interesting point is, however, that overall 
...
wonder what the effect of more categories is on the lenient categorization\index{categorization!lenient}
